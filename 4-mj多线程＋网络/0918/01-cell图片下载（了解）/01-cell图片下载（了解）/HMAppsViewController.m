 ...
 *  存放所有的下载操作（url是key，operation对象是value）
 // 控制重复下载， 一对一的关系用字典. 其实一个url就对应一个operation
 */
...
/** 在返回cell之前主线程中下载图片的问题
*  1.会阻塞主线程 - 影响用户体验
*  2.重复下载 - 浪费流量，浪费时间，影响用户体验
*  // 保证：1张图片只下载1次
 // 最好别用nsthread, 最好用nsoperation
*/
    ...
    if (image) { // 说明图片已经下载成功过（成功缓存）
        ...
        // 显示占位图片（为了解决cell循环利用的时候，新的还没下载完的时候会显示原来cell的图片）
...
//    typeof(100) age3 = 30; // int age3 = 30;
    ...
    operation = [NSBlockOperation blockOperationWithBlock:^{
        ...
        NSData *data = [NSData dataWithContentsOfURL:url]; // 下载 ； 耗时操作
        UIImage *image = [UIImage imageWithData:data]; // NSData -> UIImage 耗时操纵
        
        // 回到主线程
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
            // 存放图片到字典中
            if (image) { // 这句是防止下载失败，报错， 因为字典不能传nil。
    ...
    // 添加操作到队列中 ; 这个不需要关心， 没用的东西会自动从queue中移除
    [self.queue addOperation:operation];
    
    // 添加到字典中 (这句代码为了解决重复下载)
    // 图片越来越多，
#warning 这个字典越来越大的问题如何解决？ 而且还有一个问题， 下载一次失败了， 以后再都不会下载了;
    // 解决办法，下完之后将operation从字典中移除。 然后存储图片， 再进到cell的时候， 判断image是否存在。
